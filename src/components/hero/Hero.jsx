...
export const Hero = () => {
  ...
  const backgroundSlides = [
    ...
    {
      id: 2,
      image: "https://images.unsplash.com/photo-1571974599782-87624638275f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
      title: "Expert Piano Lessons",
  ...
  const features = [
    {
      ...
      description: "Customized lesson plans tailored to your musical goals and skill level",
      icon: <Person className="text-3xl xs:text-4xl text-purple-400" />,
      features: ["One-on-one sessions", "Progress tracking", "Flexible scheduling"],
    ...
    },
    {
      ...
      description: "Learn from professionally trained musicians with years of teaching experience",
      icon: <School className="text-3xl xs:text-4xl text-blue-400" />,
      features: ["Certified professionals", "Performance experience", "Patient mentoring"],
    ...
    },
    {
      ...
      description: "State-of-the-art studios equipped with the latest music technology",
      icon: <Analytics className="text-3xl xs:text-4xl text-cyan-400" />,
      features: ["Digital recording", "Quality instruments", "Comfortable environment"],
  ...
  const stats = [
    { icon: <Groups />, number: "500+", text: "Students Trained" },
    { icon: <School />, number: "15+", text: "Expert Instructors" },
    { icon: <TrendingUp />, number: "98%", text: "Success Rate" },
    { icon: <Star />, number: "4.9/5", text: "Student Rating" }
  ];

  const instruments = [
    { name: 'Piano', icon: '🎹', description: 'Classical to contemporary piano techniques' },
    { name: 'Guitar', icon: '🎸', description: 'Acoustic, electric, and bass guitar mastery' },
    { name: 'Vocals', icon: '🎤', description: 'Vocal training and performance techniques' },
    { name: 'Violin', icon: '🎻', description: 'String instruments and orchestral training' }
  ];
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      setBookingForm({ name: '', email: '', phone: '', instrument: '', experience: 'beginner' });
    } catch (error) {
  ...
  const floatingElements = [
    { icon: '🎹', delay: 0, size: "text-4xl xs:text-5xl", color: "text-white/30" },
    { icon: '🎸', delay: 1, size: "text-5xl xs:text-6xl", color: "text-white/25" },
    { icon: '🎤', delay: 2, size: "text-3xl xs:text-4xl", color: "text-white/30" },
    { icon: '🎻', delay: 3, size: "text-4xl xs:text-5xl", color: "text-white/25" },
    { icon: '🥁', delay: 4, size: "text-5xl xs:text-6xl", color: "text-white/30" },
    { icon: '🎼', delay: 5, size: "text-3xl xs:text-4xl", color: "text-white/25" },
    { icon: '🎧', delay: 6, size: "text-4xl xs:text-5xl", color: "text-white/30" },
    { icon: '📱', delay: 7, size: "text-3xl xs:text-4xl", color: "text-white/25" },
  ];
    ...
    <section id="home" className="hero relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className={`absolute inset-0 ${backgroundSlides[currentBgIndex].overlay}`}></div>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 relative z-10">
        ...
        <div className="text-center mb-8 sm:mb-12 lg:mb-16 max-w-4xl mx-auto">
          ...
          >
            ...
            >
              Transform your musical journey with professional guidance. Learn piano, guitar, and vocals from world-class instructors in a supportive, innovative learning environment.
            </motion.p>
            ...
            >
              ...
              <motion.button
                ...
                className="bg-white/20 backdrop-blur-md border border-white/30 hover:bg-white/30 text-white font-bold py-3 xs:py-4 px-6 xs:px-8 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 text-sm xs:text-base"
              >
        ...
        <motion.div
          ...
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 xs:gap-6 sm:gap-8 max-w-6xl mx-auto"
        >
          {features.map((feature, index) => (
            ...
            >
              ...
              <h3 className="text-lg xs:text-xl font-bold text-white mb-2 xs:mb-3">{feature.title}</h3>
              <p className="text-white/90 mb-3 xs:mb-4 leading-relaxed text-sm xs:text-base">{feature.description}</p>
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4 xs:mb-6">
              <h3 className="text-xl xs:text-2xl font-bold text-gray-900">Start Your Musical Journey</h3>
              <button
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-3 xs:space-y-4 text-black">
              <input
                type="text"
              ...
              <select
                ...
                onChange={(e) => setBookingForm({...bookingForm, instrument: e.target.value})}
                className="w-full p-2 xs:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm xs:text-base"
                required
              >
      ...
      {isContactModalOpen && (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4 xs:mb-6">
              <h3 className="text-xl xs:text-2xl font-bold text-gray-900">Contact Us</h3>
              <button
            ...
            <form onSubmit={handleContactSubmit} className="space-y-3 text-black xs:space-y-4">
              <input
                type="text"
