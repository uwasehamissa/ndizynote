...
//         {/* Hero Section */}
//         <section className="relative py-6 sm:py-5 lg:py-4 bg-gradient-to-t from-[#1e4c9c] to-[#183772] text-white overflow-hidden">
...
// Material Icons with responsive sizing
...
const LoadingIcon = ({ className = "" }) => (
  <span className={`material-icons animate-spin ${className}`}>
    <LocalDining />
  </span>
);
...
const SkipNextIcon = ({ className = "" }) => (
  <span className={`material-icons ${className}`}>
    <SkipNext />
  </span>
);
const SkipPreviousIcon = ({ className = "" }) => (
  <span className={`material-icons ${className}`}>
    <SkipPrevious />
  </span>
);
...
const FastRewindIcon = ({ className = "" }) => (
  <span className={`material-icons ${className}`}>
    <FastRewind />
  </span>
);
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {isMuted ? (
                    ...
                    <VolumeUpIcon className="text-white text-sm" />
                  )}
                </motion.button>
                <input
                  type="range"
...
export const UserClassesManagement = () => {
  ...
  // Calculate filteredVideos FIRST
  const filteredVideos =
    selectedCategory === "all"
      ? videos
      : selectedCategory === "tutorial"
  ...
  // Watch for when we reach index 4 and open next page
  useEffect(() => {
    if (
      lastViewedIndex === 4 && // Triggers when the 5th video (index 4) is clicked
      currentPage < Math.ceil(filteredVideos.length / itemsPerPage)
    ) {
      // Move to next page
      setCurrentPage((prevPage) => prevPage + 1);
  ...
  }, [lastViewedIndex, currentPage, filteredVideos.length]);
  ...
  const handleVideoThumbnailClick = (videoItem, index) => {
    ...
    // Track the absolute index of the video that was clicked
    const absoluteIndex = (currentPage - 1) * itemsPerPage + index;
    setLastViewedIndex(absoluteIndex);
  ...
  // Log to verify index tracking (for debugging)
  useEffect(() => {
    if (lastViewedIndex !== null) {
      console.log(`Video at absolute index ${lastViewedIndex} was clicked`);
      if (lastViewedIndex === 4) {
        console.log("🎯 Reached index 4! Moving to next page...");
      }
    }
  }, [lastViewedIndex]);
    ...
    <>
      <div className="min-h-screen bg-gradient-to-t from-[#1e4c9c] to-[#183772] text-white">
        ...
        >
          ...
          <section className="py-6">
            <div className="container mx-auto px-4">
              <div className="flex flex-wrap items-center gap-2 xsm:gap-3 sm:gap-4 justify-center">
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`flex items-center gap-1 xsm:gap-1.5 sm:gap-2 rounded-full transition-all
                      px-2 xsm:px-3 sm:px-4 md:px-5 py-1 xsm:py-1.5 sm:py-2
                      text-[10px] xsm:text-xs sm:text-sm md:text-base ${
                  ...
                  >
                    ...
                    <span
                      ...
                      px-1 xsm:px-1.5 sm:px-2 py-0.5
          ...
          {/* Videos Grid */}
          <section className="py-6 flex items-center justify-center min-h-screen">
            <div className="container mx-auto px-4 max-w-7xl">
              {loading ? (
                ...
                <div className="flex items-center justify-center min-h-[400px]">
                  <div className="text-center bg-red-500/10 border border-red-500/20 rounded-xl p-8 max-w-md mx-auto">
                    <h3 className="text-2xl font-bold text-white mb-2">
                      Error Loading Videos
                    </h3>
                    <p className="text-white/70 mb-4">{error}</p>
                ...
                <div className="flex flex-col items-center">
                  <motion.div
                    ...
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8 }}
                    className="grid gap-4 xsm:gap-5 sm:gap-6 
            grid-cols-1 xsm:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 w-full place-items-center"
                  >
                    {currentVideos.map((video, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          transition={{ duration: 0.5 }}
                          whileHover={{ scale: 1.02, y: -5 }}
                          className="bg-gradient-to-t from-[#1e4c9c] to-[#183772] text-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden w-full max-w-sm mx-auto"
                        >
                          {/* Video Thumbnail with Play Button */}
                          <div className="p-4">
                            <VideoThumbnail
