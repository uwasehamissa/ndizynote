...
const BookingCard = ({ booking, onView, onEdit, onDelete, isUserRequest }) => {
  const getExperienceColor = (experience) => {
    if (!experience) return "bg-gray-100 text-gray-800";
    switch (experience.toLowerCase()) {
      case "beginner":
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow ${
      isUserRequest ? 'border-l-4 border-l-indigo-500' : ''
    }`}>
      <div className="p-4">
        {/* Header */}
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            {isUserRequest && (
              <span className="inline-block px-2 py-0.5 bg-indigo-100 text-indigo-800 text-xs rounded-full mt-1">
                Your Booking
              </span>
            )}
          </div>
          <div className="flex flex-col items-end space-y-1">
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getExperienceColor(booking.experience)}`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 px-2 py-1 text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors flex items-center justify-center space-x-1 text-xs"
            title="View booking"
          >
          ...
          <button
            ...
            className="flex-1 px-2 py-1 text-green-600 bg-green-50 border border-green-200 rounded-md hover:bg-green-100 transition-colors flex items-center justify-center space-x-1 text-xs"
            title="Edit booking"
          >
          ...
          <button
            ...
            className="flex-1 px-2 py-1 text-red-600 bg-red-50 border border-red-200 rounded-md hover:bg-red-100 transition-colors flex items-center justify-center space-x-1 text-xs"
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
                ...
                <p className="text-gray-600 text-sm sm:text-base">
                  {searchTerm || filterByUserEmail || filterExperience !== "all" || filterInstrument !== "all" || filterStatus !== "all"
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {currentRequests.map((booking) => {
                        const isUserOwnRequest = isUserRequest(booking.email);
                        return (
                          <tr
                            key={booking._id || booking.id || Math.random()}
                            className={`hover:bg-gray-50 transition-colors ${
                              isUserOwnRequest ? 'bg-indigo-50' : ''
                            }`}
                          >
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {currentRequests.map((booking) => (
                  <BookingCard
                    ...
                    isUserRequest={isUserRequest(booking.email)}
                  />
                ))}
              </div>
            )}
