...
//             className="p-1 bg-gradient-to-t from-blue-500 to-blue-700 transition-colors"
//             title="View details"
//           >
...
//             className="p-1 bg-gradient-to-t from-green-500 to-green-700 transition-colors"
//             title="Edit"
//           >
...
//             className="p-1 bg-gradient-to-t from-red-500 to-red-700 transition-colors"
//             title="Delete"
//           >
...
export const MyTestimonialManagement = () => {
  ...
  // Sort testimonials by latest (newest first)
  ...
  const fetchMyTestimonials = async (page = 1) => {
    ...
    try {
      ...
      try {
        const response = await api.get(
          `/testimonials?page=${page}&limit=10&email=${encodeURIComponent(userEmail)}&sort=-createdAt`,
        );

        if (response.data && response.data.success) {
          const testimonialsData = response.data.data || [];
          // Sort by latest (newest first)
          const sortedData = sortByLatest(testimonialsData);
          ...
          // Check if we've reached the threshold to open next page
          if (viewMode === "table" && testimonialsData.length >= 5) {
            // When in list/table view and we have 5 or more items
            navigate("/next-page"); // Replace with your actual next page route
          } else if (viewMode === "grid" && testimonialsData.length >= 6) {
            // When in grid view and we have 6 or more items
            navigate("/next-page"); // Replace with your actual next page route
          }
        } else {
          // Handle different response structure
          ...
          // Check threshold for opening next page
          if (viewMode === "table" && testimonialsData.length >= 5) {
            navigate("/next-page");
          } else if (viewMode === "grid" && testimonialsData.length >= 6) {
            navigate("/next-page");
  ...
  const handleViewModeChange = (mode) => {
    ...
    // Check threshold when switching view modes
    if (mode === "table" && testimonials.length >= 5) {
      navigate("/next-page");
    } else if (mode === "grid" && testimonials.length >= 6) {
      navigate("/next-page");
    }
  };
                                ...
                                <td className="px-3 py-4 whitespace-nowrap sm:px-6">
                                  <div className="flex items-center">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <div className="lg:hidden text-xs text-gray-500 mt-1">
                                        {testimonial.createdAt?.slice(0, 10) || "N/A"}
                                      </div>
                                    </div>
                                  </div>
                                </td>
