...
//   const getMockData = () => {
//     const mockEmail = userEmail || "demo@example.com";
//     return [
//       {
//         id: 1,
//         name: "Sarah Johnson",
...
//   const isUserBooking = (bookingEmail) => {
//     return userEmail && bookingEmail && bookingEmail.toLowerCase() === userEmail.toLowerCase();
//   };

//   // Filter bookings based on search and filters - UPDATED to include user email filtering
...
//   const userBookingsCount = userEmail 
//     ? bookings.filter(b => isUserBooking(b?.email)).length 
//     : 0;
//   const userPendingBookings = userEmail 
//     ? bookings.filter(b => isUserBooking(b?.email) && b?.status === 'pending').length 
//     : 0;
...
//                   {userEmail ? `User Email: ${userEmail}` : "Manage music lesson bookings and schedules"}
//                 </p>
//               </div>
//             </div>
...
//                       <Person className="text-sm" />
//                       <span className="text-sm">Showing your bookings only</span>
//                     </div>
//                   )}
//                 </div>
//                 <button
...
//                   <p className="text-xl lg:text-2xl font-bold text-gray-100">{userBookingsCount}</p>
//                 </div>
//                 <div className="w-10 h-10 lg:w-12 lg:h-12 bg-green-100 rounded-xl flex items-center justify-center">
//                   <Person className="text-green-600 text-lg lg:text-xl" />
//                 </div>
//               </div>
//             </div>
...
//                   <p className="text-xl lg:text-2xl font-bold text-gray-100">{filteredBookings.length}</p>
//                 </div>
...
//               const isUserOwnBooking = isUserBooking(booking.email);
...
//                     isUserOwnBooking ? 'border-l-4 border-l-indigo-500' : ''
//                   }`}
//                 >
//                   <div className="p-4 lg:p-6">
//                     {/* Header */}
...
//                       <div className="flex items-center space-x-3">
//                         <Email className={`text-sm ${isUserOwnBooking ? 'text-indigo-500' : 'text-gray-400'}`} />
//                         <span className={`text-sm ${isUserOwnBooking ? 'text-indigo-600 font-medium' : 'text-gray-600'} truncate`}>
//                           {booking.email || 'N/A'}
//                         </span>
//                       </div>
//                       <div className="flex items-center space-x-3">
...
//                   {filterByUserEmail ? "No Bookings Found for Your Email" : "No Bookings Found"}
//                 </h3>
...
//                       {bookingForm.email}
//                       {isUserBooking(bookingForm.email) && (
...
//                       onChange={handleEditInputChange}
//                       className={`w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 ${
//                         isUserBooking(editForm.email) ? 'border-indigo-300' : ''
//                       }`}
//                     />
//                     {isUserBooking(editForm.email) && (
...
//                         </p>
//                         {isUserBooking(selectedBooking.email) && (
//                           <span className="px-2 py-1 bg-indigo-100 text-indigo-800 text-xs rounded-full">
//                             Your Email
//                           </span>
//                         )}
...
export const MyBookings = () => {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [userEmail, setUserEmail] = useState(null);
  const [filterByUserEmail, setFilterByUserEmail] = useState(true); // Set to true by default
  ...
  useEffect(() => {
    ...
    if (emailFromCookie) {
      ...
      // Automatically filter by user email
      setFilterByUserEmail(true);
    }
  }, []);
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      // Ensure the email is set from cookies
      const formData = {
        ...bookingForm,
        email: userEmail || bookingForm.email
      };
  ...
  const resetFilters = () => {
    ...
    setFilterStatus('all');
    setFilterDate('');
    // Don't reset user email filter as it's the primary filter
  };
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 gap-4 sm:gap-0">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-100 mt-1">
                  {userEmail ? (
                    <>
                      Showing bookings for: <span className="font-semibold text-yellow-300">{userEmail}</span>
                    </>
                  ) : (
                    "Manage your music lesson bookings"
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              {userEmail && (
                ...
                >
                  <Person className="text-sm" />
                  <span className="text-sm">{filterByUserEmail ? 'My Bookings' : 'All Bookings'}</span>
                </button>
              )}
              <motion.button
                onClick={() => setShowCreateModal(true)}
        ...
        <div className="p-4 lg:p-6">
          ...
          <div className="bg-gradient-to-t from-[#1e4c9c] to-[#183772] text-white rounded-2xl shadow-lg border border-gray-200 p-4 lg:p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="relative lg:col-span-2">
                ...
                <input
                  type="text"
                  placeholder={filterByUserEmail ? `Search your bookings...` : "Search bookings..."}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200"
                />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
            ...
            <div className="bg-gradient-to-t from-[#1e4c9c] to-[#183772] text-white rounded-2xl shadow-lg border border-gray-200 p-4 lg:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-white">Your Bookings</p>
                  <p className="text-xl lg:text-2xl font-bold text-gray-100">{userBookingsCount}</p>
                </div>
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-green-100 rounded-xl flex items-center justify-center">
                  <Person className="text-green-600 text-lg lg:text-xl" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6">
            {filteredBookings.length > 0 ? filteredBookings.map((booking) => {
              ...
              <div className="col-span-full text-center py-12">
                <CalendarToday className="mx-auto text-gray-400 text-6xl mb-4" />
                <h3 className="text-xl font-semibold text-gray-900 mb-2">
                  {filterByUserEmail ? "No Bookings Found" : "No Bookings Found"}
                </h3>
                <p className="text-gray-600 mb-6">
                  {filterByUserEmail && userEmail
                    ? `No bookings found for ${userEmail}. Create your first booking!`
              ...
              <form onSubmit={handleCreateSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {userEmail && (
                      <p className="text-xs text-indigo-600 mt-1">
                        Using email from cookies: {userEmail}
                      </p>
                    )}
                  </div>
